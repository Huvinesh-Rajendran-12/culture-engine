...
from backend.mind.store import init_db, list_drones, load_task, load_task_trace, save_mind


def _init_db(tmp_dir: str) -> Path:
    db_path = Path(tmp_dir) / "test.db"
    init_db(db_path).close()
    return db_path


def _create_mind(db_path: Path, name: str = "TestMind") -> MindProfile:
...
class TestDelegateHappyPath(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.db_path = _init_db(self.tmp)
        self.mind = _create_mind(self.db_path)
    ...
    async def test_happy_path(self, _mock_tools, mock_run_agent):
        ...
        # Stateless runtime: task/trace are not persisted in SQLite.
        task_id = events[0]["content"]["task_id"]
        self.assertIsNone(load_task(self.db_path, self.mind.id, task_id))
        self.assertIsNone(load_task_trace(self.db_path, self.mind.id, task_id))
...
class TestDelegateAgentErrorResult(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_error_result_sets_failed(self, _mock_tools, mock_run_agent):
        ...
        # Stateless runtime: failed task is not persisted.
        task_id = events[0]["content"]["task_id"]
        self.assertIsNone(load_task(self.db_path, self.mind.id, task_id))
...
class TestDelegateAgentException(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_exception_sets_failed_and_traces(self, _mock_tools, mock_run_agent):
        ...
        # Stateless runtime: trace is not persisted.
        task_id = events[0]["content"]["task_id"]
        self.assertIsNone(load_task_trace(self.db_path, self.mind.id, task_id))
...
class TestDelegateEventLimit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stream_event_limit(self, _mock_tools, mock_run_agent):
        # tool_registry is yielded before run_agent events and counts toward
        # non-text-delta events. So we need enough non-text-delta events from
        # run_agent to push over the limit.
        over_limit = [
...
class TestDroneTraceOnFailure(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_drone_trace_saved_on_failure(
        ...
        # Stateless runtime: drone records/traces are not persisted.
        self.assertEqual(list_drones(self.db_path, self.mind.id, task_id), [])
