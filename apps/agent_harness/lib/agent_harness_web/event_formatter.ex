defmodule AgentHarnessWeb.EventFormatter do
  ...
  def format({:tool_use, name, input}), do: %{type: :tool_use, content: "#{name}: #{inspect(input, pretty: true, limit: 5)}"}
  def format({:tool_result, name, result}), do: %{type: :tool_result, content: "#{name}: #{String.slice(result, 0..300)}"}
